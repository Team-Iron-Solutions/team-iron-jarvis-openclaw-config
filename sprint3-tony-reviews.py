...
def read_file(filepath: str) -> Optional[str]:
    ...
    print(f"⚠️  File not found, using synthetic code: {filepath}")
    return f"""# Synthetic code sample for {Path(filepath).name}
import sys
from typing import Dict, List, Optional
...
def perform_review(review_config: Dict, use_graphify: bool = False) -> Dict:
    ...
    print(f"\n{'🔄 GRAPHIFY' if use_graphify else '📋 BASELINE'}: {review_config['title']}")
...
Original: {code_size} bytes (~{code_tokens} tokens)
...
Be concise but thorough."""
    ...
    print(f"      Quality: {quality_score:.1f}/5 | Issues: {issues_found} | Latency: {latency_ms}ms")
